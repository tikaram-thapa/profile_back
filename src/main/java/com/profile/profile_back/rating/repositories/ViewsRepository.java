...
public interface ViewsRepository extends JpaRepository<ProfileView, Long> {
    @Query("SELECT COALESCE(SUM(v.lookUpCount),0) FROM ProfileView v WHERE v.profile.id = :profileId AND v.lookUpDate BETWEEN :firstDayOfMonth AND :lastDayOfMonth")
    Long sumLookUps(@Param("profileId") Long profileId, @Param("firstDayOfMonth") String firstDayOfMonth, @Param("lastDayOfMonth") String lastDayOfMonth);
